...
#imports the file
forecastData <- read.xlsx(x, 1,header = FALSE)
...
forecastData$Account <- 0
forecastData$Account <- as.numeric(forecastData$Account)

forecastData = forecastData[,c(10,1,2,3,4,5,6,7,8,9)]

forecastData$kWh <- round(predict(prune.sf,forecastData),digits=0)
